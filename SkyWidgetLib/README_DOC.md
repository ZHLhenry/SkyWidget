...
- **SkyIconFontsLib** — IconFont 图标字体统一管理入口，封装 [Android-Iconics](https://github.com/mikepenz/Android-Iconics) 三方库，支持 iconfont.cn 导出的字体文件自动解析，支持多字体注册与按前缀自动路由
...
基于 [Android-Iconics](https://github.com/mikepenz/Android-Iconics) 5.5.0 构建，支持多字体注册与按前缀自动路由，消费者无需感知 Iconics 类型体系。
...
| 方法 | 说明 |
|------|------|
| `initRegister(Context, List<String>)` | 初始化并注册字体，应在 `Application.onCreate` 调用。未传路径时自动使用库内置默认字体 `fonts/sky_iconfont.ttf` |
| `isInitDone()` | 是否已完成初始化 |
| `getRegisteredFonts()` | 获取所有已注册字体的摘要信息列表 `List<FontInfo>` |
| `isIconExists(String)` | 判断图标是否存在 |
| `getFormattedIconName(String, String?)` | 获取 formattedName（如 `{skyshouye}`），可选指定字体名称 |
| `style(Spanned)` | 对文本进行图标样式处理，自动根据前缀路由到对应字体，支持多字体混用 |
| `drawable(Context, String, String?)` | 根据名称创建图标 Drawable，可选指定字体名称 |
| `getSkyIconFontInfoJson(String?)` | 输出指定字体的完整信息（JSON），可选指定字体名称 |

**FontInfo 数据类：**

| 属性 | 类型 | 说明 |
|------|------|------|
| `fontName` | `String` | 字体名称（JSON 中的 name 字段） |
| `mappingPrefix` | `String` | 图标前缀（css_prefix_text 前 3 个字符） |
| `iconCount` | `Int` | 该字体包含的图标数量 |

**字体查找规则（`resolveFont` 内部逻辑）：**
1. `fontName` 作为 `fontName` 精确匹配（JSON 中的 name 字段）
2. `fontName` 作为 `mappingPrefix` 匹配（css_prefix_text 前 3 个字符）
...
```
在消费者项目的 `settings.gradle.kts` 中添加阿里云效 Maven 仓库：
...
```kotlin
// 1. Application.onCreate 中初始化（不传路径，自动使用内置默认字体）
class MyApp : Application() {
    override fun onCreate() {
        super.onCreate()
        SkyIconFontsLib.initRegister(this)  // 自动加载 fonts/sky_iconfont.ttf
    }
}
...
// 4. 检查图标是否存在
if (SkyIconFontsLib.isIconExists("skyshouye")) { ... }
...
```kotlin
// 1. Application.onCreate 中注册多个字体（第一个为默认字体）
class MyApp : Application() {
    override fun onCreate() {
        super.onCreate()
        SkyIconFontsLib.initRegister(this, listOf(
            "fonts/testSky_iconfont.ttf",   // 默认字体（第一个注册）
            "fonts/adb_iconfont.ttf"         // 第二个字体
        ))
    }
}

// 2. 使用默认字体（不传 fontName）
imageView.setImageDrawable(SkyIconFontsLib.drawable(context, "testSkyshouye"))

// 3. 指定字体创建 Drawable（按 fontName 或 mappingPrefix 查找）
imageView.setImageDrawable(SkyIconFontsLib.drawable(context, "adbdingwei", "adb-iconfont"))

// 4. 指定字体获取 formattedName
val text = "${SkyIconFontsLib.getFormattedIconName("adbtixi", "adb-iconfont")} 详情"

// 5. style() 多字体混用（自动按 {prefix_name} 前缀路由到对应字体）
...
textView.text = SkyIconFontsLib.style(SpannableString(mixedText))

// 6. 查看所有已注册字体
val fonts = SkyIconFontsLib.getRegisteredFonts()
fonts.forEach { info ->
    println("字体: ${info.fontName} | prefix: ${info.mappingPrefix} | 图标数: ${info.iconCount}")
}
```
...
assets/
└── fonts/
    ├── sky_iconfont.ttf      # 库内置默认字体（initRegister 不传路径时自动加载）
    ├── sky_iconfont.json     # iconfont.cn 导出的同名 JSON
    ├── custom_iconfont.ttf   # 自定义字体（多字体场景）
    └── custom_iconfont.json  # 对应的 JSON 文件
```
...
- `initRegister()` 不传 `ttfPaths` 参数时，自动加载库内置默认字体 `fonts/sky_iconfont.ttf`；传入路径列表时，第一个注册的字体为默认字体
...
- 多字体混用时，`style()` 方法自动根据 `{prefix_name}` 中的前缀路由到对应已注册字体，无需手动指定 `fontName`
- `getFormattedIconName()`、`drawable()`、`getSkyIconFontInfoJson()` 均支持可选的 `fontName` 参数，按 fontName 精确匹配 → mappingPrefix 匹配 → 默认字体的优先级查找
